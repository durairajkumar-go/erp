...
<div class="col-sm-12" >
    <div class="panel panel-bd" data-index="0">
        <div class="panel-heading">
            ...
            @if($add_role=='1')    
            <a href="{{ url('process/create') }}" class="btn btn-sm btn-primary pull-right hvr-buzz-out fa fa-pencil" style="color: #fff"  onclick="clickAndDisable(this);" ></a>
            @endif
            </div>
        </div>
...
//buttons option
    $buttons ="['pageLength'";
    if($copy_role=='1')
    $buttons .=",{ extend: 'copy', title: '".trans('messages.process')."',exportOptions: { columns: ':visible' } }";
    if($csv_role=='1')
    $buttons .=",{ extend: 'csv', title: '".trans('messages.process')."', exportOptions: { columns: ':visible' } }";
    if($excel_role=='1')
    $buttons .=",{ extend: 'excel', title: '".trans('messages.process')."', exportOptions: { columns: ':visible' }}";
    if($pdf_role=='1')
    $buttons .=",{ extend: 'pdf', title: '".trans('messages.process')."' , exportOptions: { columns: ':visible'}}";
    if($print_role=='1')
    $buttons .=",{ extend: 'print', title: '".trans('messages.process')."', exportOptions: { columns: ':visible'}}";
    $buttons .=",{
        extend: 'colvis',
