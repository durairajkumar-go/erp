...
<table id="users-table" class="table table-bordered table-striped table-hover dataTable no-footer" role="grid" style="width: 100%">
        <thead>
            <tr>
                ...
                <th>{{ trans('messages.background color') }} </th>
...
//buttons option
    $buttons ="['pageLength'";
    if($copy_role=='1')
    $buttons .=",{ extend: 'copy', title: '".trans('messages.dashboard')."',exportOptions: { columns: ':visible' } }";
    if($csv_role=='1')
    $buttons .=",{ extend: 'csv', title: '".trans('messages.dashboard')."', exportOptions: { columns: ':visible' } }";
    if($excel_role=='1')
    $buttons .=",{ extend: 'excel', title: '".trans('messages.dashboard')."', exportOptions: { columns: ':visible' }}";
    if($pdf_role=='1')
    $buttons .=",{ extend: 'pdf', title: '".trans('messages.dashboard')."' , exportOptions: { columns: ':visible'}}";
    if($print_role=='1')
    $buttons .=",{ extend: 'print', title: '".trans('messages.dashboard')."', exportOptions: { columns: ':visible'}}";
    $buttons .=",{
        extend: 'colvis',
...
$(function() {
   ...
   var dt= $('#users-table').DataTable({
        ...
        ajax: '{!! route('dashboard_data.data') !!}',
        columns: [
            { data: 'id', name: 'id' },
            { data: 'name', name: 'name' },
            { data: 'icon', name: 'icon' },
