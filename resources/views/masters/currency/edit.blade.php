...
<div class="col-sm-12" >
    <div class="panel panel-bd" data-index="0">
        <div class="panel-heading">
            ...
            @if($add_role=='1')    
              <a href="{{ url('currency/create') }}" class="btn btn-sm btn-primary pull-right hvr-buzz-out fa fa-pencil" style="color: #fff"  onclick="clickAndDisable(this);"></a>
             @endif 
...
{!! Form::open(array('route' => ['currency.update', $data->id] , 'method' => 'PUT'),['class' => 'form']) !!}

 <?php
    $items = App\Model\Masters\Currency::where('record_status','1')->orderBy('id')->pluck('name','exchange_rate','symbol','default','ledger');
 ...
 ?>
                        ...
                        <div class="form-group">
                           <div class="styled-input col-sm-6 col-md-3 col-lg-3 {{ $errors->first('exchange_rate', 'has-error') }}">
                            {{ Form::text('exchange_rate', $data->exchange_rate, ['class' => 'form-control',(($errors->first('exchange_rate')) ? 'autofocus': null),'onfocus'=>'this.value = this.value','autocomplete' => 'off','onkeypress'=>'return isNumber(event,this)']) }}
						 ...
						 <div class="form-group">
                           <div class="styled-input col-sm-6 col-md-3 col-lg-3 {{ $errors->first('symbol', 'has-error') }}">
                            {{ Form::text('symbol', $data->symbol, ['class' => 'form-control',(($errors->first('symbol')) ? 'autofocus': null),'onfocus'=>'this.value = this.value','autocomplete' => 'off']) }}
                            {{ Form::label('symbol', trans('messages.symbol')) }}
                            </div>
                        </div>
						<div class="form-group">
                           <div class="styled-input col-sm-6 col-md-3 col-lg-3 {{ $errors->first('ledger', 'has-error') }}">
                            {{ Form::text('ledger', $data->ledger, ['class' => 'form-control',(($errors->first('ledger')) ? 'autofocus': null),'onfocus'=>'this.value = this.value','autocomplete' => 'off']) }}
